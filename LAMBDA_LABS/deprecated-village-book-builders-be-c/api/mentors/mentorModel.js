...
const findById = async (id) => {
  return db('mentors').where({ id }).first().select('*');
};

const create = async (profile) => {
  return db('mentors').insert(profile).returning('*');
};

const update = (id, profile) => {
  console.log(profile);
  return db('mentors').where({ id: id }).first().update(profile).returning('*');
};

const remove = async (id) => {
  return await db('mentors').where({ id }).del();
};
